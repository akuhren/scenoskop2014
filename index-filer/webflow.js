...
Webflow.define('navbar', function ($, _) {
  ...
  function select(i, el) {
    ...
    // Check for section hash and add to anchors
    if (href.indexOf('#') === 0 && validHash.test(href)) {
      anchors.push({ el: el, sec: $(href), active: false });
      return;
    }
  ...
  function scroll() {
    ...
    _.each(anchors, function (anchor) {
      ...
      var active = (sec.is(':visible') &&
        top + height - offset >= viewTop &&
        top + offset <= viewTop + viewHeight);
      if (anchor.active === active) return;
      anchor.active = active;
      el.toggleClass(linkCurrent, active);
      if (designer) el[0].__wf_current = active;
    });
  }
